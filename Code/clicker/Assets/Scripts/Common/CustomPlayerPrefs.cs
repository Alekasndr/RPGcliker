...
public sealed class CustomPlayerPrefs 
{
    ...
    public static void SetObjectsValue<T>(string key, List<T> value, bool saveImmediately = false)
        where T : class
    {
        string objectValue = (value == null) ? (string.Empty) : (JsonHelper.ToJson(value.ToArray()));
    ...
    public static List<T> GetObjectsValue<T>(string key) where T : class
    {
        string savedObjectValue = GetString(key);

        return (string.IsNullOrEmpty(savedObjectValue))
            ? (null)
            : (new List<T>(JsonHelper.FromJson<T>(savedObjectValue)));
    }
